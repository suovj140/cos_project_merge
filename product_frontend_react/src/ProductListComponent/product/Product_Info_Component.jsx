...
function ProductInfoComponent(props){
    ...
    //사이즈 선택후 
    function chang(e){
        console.log('productop4', productop);
    ...
    function savecart(e){
        e.preventDefault();
        console.log('장바구니shopcart',shopcart);
        let cart={
            user_email:"licsh12@gmail.com",
    ...
    const settitle = Array.from(new Set(product && product.map(option => option.product_title)));
    const setprice = Array.from(new Set(product && product.map(option => option.product_price)));
    const setcontent = Array.from(new Set(product && product.map(option => option.product_content)));
    const setmeterial = Array.from(new Set(product && product.map(option => option.product_material)));
                ...
                <div className = "productinfo_detail">
                    <div className = "detail_info">
                        <div className = "detail_info_title">
                            <div className = "title" style = {{fontSize : "21px"}}>{settitle}</div>
                        ...
                        <div className = "detail_info_content">
                            <div className = "detail_info_content_btn">
                                <Button onClick = {() => setproduct_info(setcontent )}>상품 설명</Button>
                                <Button onClick = {() => setproduct_info(text)}> - 배송 반품 -</Button>
                                <Button onClick = {() => setproduct_info(setmeterial)}>상세 정보</Button>
                            </div>
